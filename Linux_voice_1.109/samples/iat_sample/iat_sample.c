...
void run_iat(const char* audio_file, const char* session_begin_params)
{
	...
	printf("=============================================================\n");
	printf("%s\n",rec_result);
	f_result = fopen("iat_result.txt", "wb");
	if (NULL == f_result)
	{
		printf("open %s error.\n", "iat_result.txt");		
	}
	fwrite(rec_result, sizeof(rec_result), 1, f_result);
	fclose(f_result);
	printf("=============================================================\n");
...
int main(int argc, char* argv[])
{
	...
	const char* login_params			=	"appid = 5804f149, work_dir = ."; // 登录参数，appid与msc库绑定,请勿随意改动56ee43d0
